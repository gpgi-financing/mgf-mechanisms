function [ Y ] = outcomeSummarySimple(S,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease)
...
Yb=transpose(ResultingAllocations)*A+reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions*(sum(PureFunds)+1/(1-r)*sum(FreeFunds))*NetCrudeOilImports2017Normalised+AggregateMitigationBenefitsDueToKerosineConsumptionDecrease*ProportionsOfSCC*(sum(PureFunds)+1/(1-r)*sum(FreeFunds));
TaxBurden=sum(transpose(f01+f02+f11+f12+f10+f21+f22+f20))+sum(f01+f02+f11+f12+f10+f21+f22+f20);
Yc=TaxBurden.*transpose(C);
Y(1)=(sum(Yb-Yc)+sum(FreeFunds*r/(1-r))+sum(TaxBurden.*cLossOfRents)); %aggregate payoffs
Y(2)=(sum(ResultingAllocations)); %aggregate money raised for GPGFs
Y(3)=((sum(ResultingAllocations)+r/(1-r)*sum(totalFreeFunds)));    %aggregate money collected, since totalFreeFunds=(1-r)*moneyCollectedByParticipantsWithoutRightToInfluence and r*moneyCollectedByParticipantsWithoutRightToInfluence is the amount that is collected but not given to GPGIs.
...
TaxBurden=sum(transpose(f01+f02+f11+f12+f10+f21+f22+f20))+sum(f01+f02+f11+f12+f10+f21+f22+f20);
Yc=TaxBurden.*transpose(C);
Y(1)=(sum(Yb-Yc)+sum(TaxBurden.*cLossOfRents)); %aggregate payoffs
Y(2)=sum(Z); %aggregate money raised for GPGFs
Y(3)=sum(Z);    %money collected
end
if sum(Funds)>=0.9
for i=1:N
...
TaxBurden=sum(transpose(f01+f02+f11+f12+f10+f21+f22+f20))+sum(f01+f02+f11+f12+f10+f21+f22+f20);
Yc=TaxBurden.*transpose(C);
Y(1)=(sum(Yb-Yc+r*Funds)+sum(TaxBurden.*cLossOfRents)); %aggregate payoffs+rents shifted
Y(2)=(sum(Z)); %aggregate money raised for GPGFs
Y(3)=(sum(Funds));    %money collected
end
