...
def loadPage():
    ...
    for gameInfo in gameInfos:
        ...
        for strat in strategyData:
            ...
            strategies[strat[6]] = Strategy(proportionNonAllocated, proportionDirect, proportionMatching, directSplit, matchingComp)
    ...
    def receiveStrategy():
        ...
        cur.execute("UPDATE TABLE games SET currPlayer = %s, payoffs = %s WHERE name = %s", (game.currPlayer, game.payoffs, game.name))
        cur.execute("UPDATE TABLE strategies SET proportionNonAllocated = %s, proportionDirect = %s, proportionMatching = %s, directSplit = %s, matchingComp = %s WHERE game = %s AND player = %s",
                    (strategy.proportionNonAllocated, strategy.proportionDirect, strategy.proportionMatching, createStringOfInts(strategy.directSplit), createStringOfInts(strategy.matchingComp), game.name, context.uid))
