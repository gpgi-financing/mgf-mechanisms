  function [ Y ] = PayoffsSimpleCORSIA(S,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease)
...
ResultingAllocations=allocationsSimple(GPGIs,totalFreeFunds); %we first compute the allocations as if no money was retained.
Yb=(1-r)*transpose(ResultingAllocations)*A+reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions*(sum(PureFunds)+sum(FreeFunds))*NetCrudeOilImports2017Normalised+AggregateMitigationBenefitsDueToKerosineConsumptionDecrease*ProportionsOfSCC*(sum(PureFunds)+sum(FreeFunds));
...
if sum(ParticipantsWithInfluence+ParticipantsWithoutInfluence)>0&&ParticipantsWithInfluence(3)+ParticipantsWithoutInfluence(3)==0 %i.e. if the EU does not participate even though one other player participates.
    Y(3)=-10;
else
end