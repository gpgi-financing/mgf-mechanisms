...
for i=1:N
   if floor(s(i))==2
    ...
    orderedind=sort(ind);
    PMF2s(orderedind(1),orderedind(2))=PMF2s(orderedind(1),orderedind(2))+m(i)*PureFunds(i)*10*(s(i)-2);   % We recall that we encoded 'giving all to the PMF3 with the three highest ranked GPGPIs' as '3.1'.
    GPGIs(ind(1))=GPGIs(ind(1))+m(i)*p(i)*PureFunds(i)*(1-10*(s(i)-2)); %p(i) is the proportion of the money not given to PMFs given directly to the most preferred GPGI.
    GPGIs(ind(2))=GPGIs(ind(2))+m(i)*(1-p(i))*PureFunds(i)*(1-10*(s(i)-2));
   end
      if floor(s(i))==3
    [maxvalues, ind] = maxk(A(:,i), 3);
    orderedind=sort(ind);
    PMF3s(orderedind(1),orderedind(2),orderedind(3))=PMF3s(orderedind(1),orderedind(2),orderedind(3))+m(i)*PureFunds(i)*10*(s(i)-3);   
          GPGIs(ind(1))=GPGIs(ind(1))+m(i)*p(i)*PureFunds(i)*(1-10*(s(i)-3));
    GPGIs(ind(2))=GPGIs(ind(2))+m(i)*(1-p(i))*PureFunds(i)*(1-10*(s(i)-3));
      end
            if floor(s(i))==4
              [maxvalues, ind] = maxk(A(:,i), 4);
    orderedind=sort(ind);
    PMF4s(orderedind(1),orderedind(2),orderedind(3),orderedind(4))=PMF4s(orderedind(1),orderedind(2),orderedind(3),orderedind(4))+m(i)*PureFunds(i)*10*(s(i)-4);   
          GPGIs(ind(1))=GPGIs(ind(1))+m(i)*p(i)*PureFunds(i)*(1-10*(s(i)-4));
    GPGIs(ind(2))=GPGIs(ind(2))+m(i)*(1-p(i))*PureFunds(i)*(1-10*(s(i)-4));
            end
   ...
   GPGIs(ind(1))=GPGIs(ind(1))+m(i)*p(i)*PureFunds(i);
      ...
      totalFreeFunds=totalFreeFunds+(1-r)*(1-m(i))*PureFunds(i);
end

ResultingAllocations=allocationsFromHPMFsL(GPGIs,PMF2s,PMF3s,PMF4s,totalFreeFunds,h);
...
TaxBurden=sum(transpose(f01+f02+f11+f12+f10+f21+f22+f20))+sum(f01+f02+f11+f12+f10+f21+f22+f20);
Yc=TaxBurden.*transpose(C);
Y=Yb-Yc+r*Funds-joyOfGivingToGPGI+r*PureFunds;
end
end
end
