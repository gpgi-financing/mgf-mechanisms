function[Y]=allocationsFromHPMFsL(GPGIs,PMF2s,PMF3s,PMF4s,totalFreeFunds,h) % GPGIs(k) is the amount given diirectly to GPGF k.
%h This is the proportion that each PMF gives as direct allocations to GPGIs.
%threshold=0.00000001;
%%%%%%%% smallBonus=0.00001; %This is just a tiny number that we endow each GPGI with so as to not have to worry about dividing by 0.
g=length(GPGIs);
Y=zeros(g,1);
