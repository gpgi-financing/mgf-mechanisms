...
AggregateMitigationBenefitsDueToKerosineConsumptionDecrease=0.74*3*0.6*(1-crossRegionalSubstitution)*36*859/(855 *1000);
...
R=0
% Here we use the assumption that the carbon offset eligibility criteria
% chosen by the EU would lead to CDM projects and REDD projects being
% funded in such a way that the effect of money used to buy up these
% offsets is equivalent to the effect of splitting the money evenly between
% the CDM and the FCPF's carbon fund
efficiencyFactorOfEmissionsTaxesOverTicketTaxes=0.9;
% C=(DWL+efficiencyFactorOfEmissionsTaxesOverTicketTaxes)*ones(N,1); % we assume that each country bears half the incidence of the taxes on its flights.
R=0;
...
payoffs=PayoffsSimpleCORSIA(s,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease);
...
payoffsk=zeros(2,1);
    ss(k)=0;
    PayoffOutside=PayoffsSimpleCORSIA(ss,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease);
    payoffsk(1)=PayoffOutside(k);
    ss(k)=1;
    PayoffGivingToGPGIs=PayoffsSimpleCORSIA(ss,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease);
    payoffsk(2)=PayoffGivingToGPGIs(k);
           [M,I]=max(payoffsk);
           if k==2&&numberOfActionProfileChanges<100 %for China we restrict responses to those involving participation
            I=2;
            M=payoffsk(2);
           end
    
           payoffsbefore=PayoffsSimpleCORSIA(s,A,F,C,r,R,reductionInOilRevenuesPerDollarRaisedViaTaxesOnFlightEmissions,AggregateMitigationBenefitsDueToKerosineConsumptionDecrease);
           ...
           ss(k)=I-1; %Here we set the action of k to k's best response.         
           end
